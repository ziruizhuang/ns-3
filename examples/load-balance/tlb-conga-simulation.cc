...
{
  // TODO: ZZR, ns-3 changed a lot from 3.25 to 3.33, need to confirm whether
  //  GetQueueSize mean current NBytes or current Packet size
  uint32_t qSize = StaticCast<RedQueueDisc> (queue)->GetCurrentSize().GetValue();
...
void
CheckQueueSize (Ptr<Queue<Packet>> queue, std::string plot, Gnuplot2dDataset *dataset)
{
...
CheckThroughput (Ptr<PacketSink> sink, std::string plot)
{
  ...
  void (&rf)(Ptr<PacketSink>, std::string) = CheckThroughput;  // ZZR: reference of overloaded function
  Simulator::Schedule (Seconds (0.001), rf, sink, plot);
...
CheckThroughput (Ptr<PacketSink> sink)
{
  ...
  void (&rf)(Ptr<PacketSink>) = CheckThroughput;  // ZZR: reference of overloaded function
  Simulator::Schedule (Seconds (0.001), rf, sink);
...
int main (int argc, char *argv[])
{
    ...
    {
        Config::SetDefault ("ns3::TcSocketBase::ECN", BooleanValue (false));
    }
   ...
   // Ipv4GlobalRoutingHelper::PopulateRoutingTables ();
    ...
    void (&rf)(Ptr<PacketSink>, std::string) = CheckThroughput;  // ZZR: reference of overloaded function
    Simulator::ScheduleNow (rf, pktSink, oss.str ());
    }
