...
void
TLBPathSelectTrace (uint32_t flowId, uint32_t fromTor, uint32_t destTor, uint32_t path,
                    bool isRandom, PathInfo pathInfo, std::vector<PathInfo> parallelPaths)
{
  NS_LOG_UNCOND ("Flow: " << flowId << " (" << fromTor << " -> " << destTor
                          << ") selects path: " << path << " at: " << Simulator::Now ());
  NS_LOG_UNCOND ("\t Is random select: " << isRandom);
  NS_LOG_UNCOND ("\t Path info: type -> "
                 << Ipv4TLB::GetPathType (pathInfo.pathType) << ", min RTT -> " << pathInfo.rttMin
                 << ", ECN Portion -> " << pathInfo.ecnPortion << ", Flow counter -> "
                 << pathInfo.counter << ", Quantified DRE -> " << pathInfo.quantifiedDre);
  ...
  for (; itr != parallelPaths.end (); ++itr)
    {
      ...
      NS_LOG_UNCOND ("\t\t Path info: "
                     << path.pathId << ", type -> " << Ipv4TLB::GetPathType (path.pathType)
                     << ", min RTT -> " << path.rttMin << ", Path size -> " << path.size
                     << ", ECN Portion -> " << path.ecnPortion << ", Flow counter -> "
                     << path.counter << ", Quantified DRE -> " << path.quantifiedDre);
    }
  NS_LOG_UNCOND ("\n");

  std::ofstream out (tlbBibleFilename.str ().c_str (), std::ios::out | std::ios::app);
  out << "Flow: " << flowId << " (" << fromTor << " -> " << destTor << ") selects path: " << path
      << " at: " << Simulator::Now () << std::endl;
  out << "\t Is random select: " << isRandom << std::endl;
  out << "\t Path info: type -> " << Ipv4TLB::GetPathType (pathInfo.pathType) << ", min RTT -> "
      << pathInfo.rttMin << ", ECN Portion -> " << pathInfo.ecnPortion << ", Flow counter -> "
      << pathInfo.counter << ", Quantified DRE -> " << pathInfo.quantifiedDre << std::endl;
  ...
  for (; itr != parallelPaths.end (); ++itr)
    {
      ...
      out << "\t\t Path info: " << path.pathId << ", type -> "
          ...
          << ", Path size -> " << path.size << ", ECN Portion -> " << path.ecnPortion
          << ", Flow counter -> " << path.counter << ", Quantified DRE -> " << path.quantifiedDre
          << std::endl;
    }
  out << std::endl;
}
...
void
TLBPathChangeTrace (uint32_t flowId, uint32_t fromTor, uint32_t destTor, uint32_t newPath,
                    uint32_t oldPath, bool isRandom, std::vector<PathInfo> parallelPaths)
{
  NS_LOG_UNCOND ("Flow: " << flowId << " (" << fromTor << " -> " << destTor
  ...
  out << "Flow: " << flowId << " (" << fromTor << " -> " << destTor
      << ") changes path from: " << oldPath << " to " << newPath << " at: " << Simulator::Now ();
  out << "\t Is random select: " << isRandom << std::endl;
  out << "\t Parallel path info: " << std::endl;
  itr = parallelPaths.begin ();
  for (; itr != parallelPaths.end (); ++itr)
...
void
RBTraceBuffer (uint32_t flowId, Time time, SequenceNumber32 revSeq, SequenceNumber32 expectSeq)
{
  NS_LOG_UNCOND ("Flow: " << flowId << " (at time: " << time << "), receives: " << revSeq
                          << ", while expecting: " << expectSeq);
  std::ofstream out (rbTraceFilename.str ().c_str (), std::ios::out | std::ios::app);
  out << "Flow: " << flowId << " (at time: " << time << "), receives: " << revSeq
      << ", while expecting: " << expectSeq << std::endl;
}

void
RBTraceFlush (uint32_t flowId, Time time, SequenceNumber32 popSeq, uint32_t inOrderQueueLength,
              uint32_t outOrderQueueLength, TcpRBPopReason reason)
{
  NS_LOG_UNCOND ("Flow: " << flowId << " (at time: " << time << "), pops: " << popSeq
...
RBTrace (void)
{
  Config::ConnectWithoutContext (
      "/NodeList/*/$ns3::TcpL4Protocol/SocketList/*/ResequenceBufferPointer/Buffer",
      MakeCallback (&RBTraceBuffer));
  Config::ConnectWithoutContext (
      "/NodeList/*/$ns3::TcpL4Protocol/SocketList/*/ResequenceBufferPointer/Flush",
...
void
install_applications (int fromLeafId, NodeContainer servers, double requestRate,
                      struct cdf_table *cdfTable, long &flowCount, long &totalFlowSize,
                      int SERVER_COUNT, int LEAF_COUNT, double START_TIME, double END_TIME,
                      double FLOW_LAUNCH_END_TIME, uint32_t applicationPauseThresh,
                      uint32_t applicationPauseTime, std::vector<std::vector<int>> &_has_sink)
{
  NS_LOG_INFO ("Install applications:");
  for (int i = 0; i < SERVER_COUNT; i++)
    {
      NS_LOG_INFO ("Install for server " + std::to_string(i) + " at rate " + std::to_string(requestRate));
      ...
      while (startTime < FLOW_LAUNCH_END_TIME)
        {
          ...
          // TODO: ZZR, disable delay for now. Try to figure out how to add it back later. /src/application/model/bulk-send-application
...
//          source.SetAttribute ("DelayTime", TimeValue (MicroSeconds (applicationPauseTime)));
          ...
          // Fixed: ZZR, may have duplicate port, so only initialize the sink once
          if (_has_sink[destServerIndex][port] == 0)
          {
            // TODO: ZZR, new ns-3 have problem binding to IPv4Address::GetAny()
            //  use explicit address instead.
            PacketSinkHelper sink ("ns3::TcpSocketFactory",
//                                 InetSocketAddress (Ipv4Address::GetAny (), port));
                                   InetSocketAddress (destAddress, port));
            std::cout << fromServerIndex << "=>" << destServerIndex << " port: " << port << " Addr: ";
            destAddress.Print(std::cout);
            std::cout << "from node id: " << servers.Get(fromServerIndex)->GetId() << " to node id: " << servers.Get(destServerIndex)->GetId();
            std::cout << std::endl;
            ApplicationContainer sinkApp = sink.Install (servers.Get (destServerIndex));
            sinkApp.Start (Seconds (START_TIME));
            sinkApp.Stop (Seconds (END_TIME));
            _has_sink[destServerIndex][port] = 1;
          }


//          NS_LOG_INFO("app install at time " + std::to_string(startTime) + std::to_string(FLOW_LAUNCH_END_TIME));
...
main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("runMode",
                "Running mode of this simulation: Conga, Conga-flow, Presto, Weighted-Presto, DRB, "
                "FlowBender, ECMP, Clove, DRILL, LetFlow",
                runModeStr);
  ...
  cmd.AddValue ("resequenceInOrderTimer", "In order queue timeout in resequence buffer",
                resequenceInOrderTimer);
  cmd.AddValue ("resequenceOutOrderTimer", "Out order queue timeout in resequence buffer",
                resequenceOutOrderTimer);
  cmd.AddValue ("resequenceInOrderSize", "In order queue size in resequence buffer",
                resequenceInOrderSize);
  cmd.AddValue ("resequenceBufferLog", "Whether enabling the resequence buffer logging system",
                resequenceBufferLog);

  cmd.AddValue ("asymCapacity",
                "Whether the capacity is asym, which means some link will have only 1/10 the "
                "capacity of others",
                asymCapacity);
  cmd.AddValue ("asymCapacityPoss", "The possibility that a path will have only 1/10 capacity",
                asymCapacityPoss);
  ...
  cmd.AddValue ("TLBECNPortionLow", "The ECN portion used in judging a good path in TLB",
                TLBECNPortionLow);
  cmd.AddValue ("TLBRunMode",
                "The running mode of TLB, 0 for minimize counter, 1 for minimize RTT, 2 for "
                "random, 11 for RTT counter, 12 for RTT DRE",
                TLBRunMode);
  ...
  cmd.AddValue ("TLBReverseACK", "Whether to enable the TLB reverse ACK path selection",
                TLBReverseACK);
  ...
  cmd.AddValue ("cloveRunMode",
                "Clove run mode, 1 for edge flowlet, 2 for ECN, 3 for INT (not yet implemented)",
                cloveRunMode);
  cmd.AddValue ("cloveHalfRTT", "Half RTT used in Clove ECN", cloveHalfRTT);
  cmd.AddValue (
      "cloveDisToUncongestedPath",
      "Whether Clove will distribute the weight to uncongested path (no ECN) or all paths",
      cloveDisToUncongestedPath);
  ...
  cmd.AddValue ("enableRandomDrop",
                "Whether the Spine-0 to other leaves has the random drop problem",
                enableRandomDrop);
  cmd.AddValue ("randomDropRate", "The random drop rate when the random drop is enabled",
                randomDropRate);

  cmd.AddValue ("blackHoleMode",
                "The packet black hole mode, 0 to disable, 1 src, 2 dest, 3 src/dest pair",
                blackHoleMode);
  ...
  cmd.AddValue ("asymCapacity2", "Whether the Spine0-Leaf0's capacity is asymmetric",
                asymCapacity2);

  cmd.AddValue ("enableLargeSynRetries", "Whether the SYN packet would retry thousands of times",
                enableLargeSynRetries);
  cmd.AddValue ("enableFastReConnection",
                "Whether the SYN gap will be very small when reconnecting", enableFastReConnection);
  ...
  if (runModeStr.compare ("Conga") == 0)
  ...
  else if (runModeStr.compare ("Weighted-Presto") == 0)
    {
      if (asymCapacity == false && asymCapacity2 == false)
        {
          NS_LOG_ERROR ("The Weighted-Presto has to work with asymmetric topology. For a symmetric "
                        "topology, please use Presto instead");
          return 0;
        }
  ...
  // Fixed: ZZR, memory bug (Conditional jump depends on uninitialized values)
  ...
  if (transportProt.compare ("Tcp") == 0)
    {
      p2p.SetQueue ("ns3::DropTailQueue", "MaxSize", QueueSizeValue ( QueueSize (std::to_string (BUFFER_SIZE) + "p")));
    }
  else
    {
          ...
          if (runMode == CONGA || runMode == CONGA_FLOW || runMode == CONGA_ECMP)
            {
              ...
              congaRoutingHelper.GetCongaRouting (leaves.Get (i)->GetObject<Ipv4> ())
                  ->AddRoute (interfaceContainer.GetAddress (1), Ipv4Mask ("255.255.255.255"),
                              netDeviceContainer.Get (0)->GetIfIndex ());
          ...
          for (int l = 0; l < LINK_COUNT; l++)
            {
                      ...
                      if (j == 0)
                        {
                          ...
                          tc.Install (netDeviceContainer.Get (1)); // Spine to Leaf Queue
                          Ptr<TrafficControlLayer> tc = netDeviceContainer.Get (1)
                                                            ->GetNode ()
              ...
              if (runMode == TLB || runMode == DRB || runMode == PRESTO ||
                ...
                {
                  auto leafToSpine = std::make_pair (i, j);
                  leafToSpinePath[leafToSpine] = netDeviceContainer.Get (0)->GetIfIndex ();

                  auto spineToLeaf = std::make_pair (j, i);
              ...
              if (runMode == DRILL)
                {
                  ...
                  Ptr<Ipv4DrillRouting> drillSpine =
                      drillRoutingHelper.GetDrillRouting (spines.Get (j)->GetObject<Ipv4> ());
  ...
  if (runMode == ECMP || runMode == PRESTO || runMode == WEIGHTED_PRESTO || runMode == DRB ||
      runMode == FlowBender || runMode == TLB || runMode == Clove)
    {
                  ...
                  if (runMode == DRB)
                  ...
                  else if (runMode == WEIGHTED_PRESTO)
                    {
                      ...
                      if (asymLink.find (std::make_pair (i, k)) != asymLink.end ())
                        {
                          drbRouting->AddWeightedPath (PRESTO_RATIO * 0.2,
                      ...
                      else
                        {
                          ...
                          drbRouting->AddWeightedPath (
                              PRESTO_RATIO, leafToSpinePath[std::make_pair (i, k)], exclusiveIPs);
                        }
                    }
                  else
                    {
  ...
  std::vector<std::vector<int>> has_sink(SERVER_COUNT*SERVER_COUNT);
  for (int hs_i = 0; hs_i< SERVER_COUNT*SERVER_COUNT;hs_i ++)
  {
    has_sink[hs_i].resize(65536, 0);
  }

  for (int fromLeafId = 0; fromLeafId < LEAF_COUNT; fromLeafId++)
    {
      install_applications (fromLeafId, servers, requestRate, cdfTable, flowCount, totalFlowSize,
                            SERVER_COUNT, LEAF_COUNT, START_TIME, END_TIME, FLOW_LAUNCH_END_TIME,
                            applicationPauseThresh, applicationPauseTime, has_sink);
    }
  ...
  flowMonitor = flowHelper.InstallAll (); // TODO: ZZR, temporary disable flow monitor
  ...
  flowMonitorFilename << id << "-1-large-load-" << LEAF_COUNT << "X" << SPINE_COUNT << "-" << load
                      << "-" << transportProt << "-";
  linkMonitorFilename << id << "-1-large-load-" << LEAF_COUNT << "X" << SPINE_COUNT << "-" << load
                      << "-" << transportProt << "-";
  tlbBibleFilename << id << "-1-large-load-" << LEAF_COUNT << "X" << SPINE_COUNT << "-" << load
                   << "-" << transportProt << "-";
  tlbBibleFilename2 << id << "-1-large-load-" << LEAF_COUNT << "X" << SPINE_COUNT << "-" << load
                    << "-" << transportProt << "-";
  rbTraceFilename << id << "-1-large-load-" << LEAF_COUNT << "X" << SPINE_COUNT << "-" << load
                  << "-" << transportProt << "-";
  ...
  if (runMode == CONGA)
    ...
    {
      flowMonitorFilename << "tlb-" << TLBHighRTT << "-" << TLBMinRTT << "-" << TLBBetterPathRTT
                          << "-" << TLBPoss << "-" << TLBS << "-" << TLBT1 << "-"
                          << TLBProbingInterval << "-" << TLBSmooth << "-" << TLBRerouting << "-"
                          << quantifyRTTBase << "-";
      linkMonitorFilename << "tlb-" << TLBHighRTT << "-" << TLBMinRTT << "-" << TLBBetterPathRTT
                          << "-" << TLBPoss << "-" << TLBS << "-" << TLBT1 << "-"
                          << TLBProbingInterval << "-" << TLBSmooth << "-" << TLBRerouting << "-"
                          << quantifyRTTBase << "-";
      tlbBibleFilename << "tlb-" << TLBHighRTT << "-" << TLBMinRTT << "-" << TLBBetterPathRTT << "-"
                       << TLBPoss << "-" << TLBS << "-" << TLBT1 << "-" << TLBProbingInterval << "-"
                       << TLBSmooth << "-" << TLBRerouting << "-" << quantifyRTTBase << "-";
      tlbBibleFilename2 << "tlb-" << TLBHighRTT << "-" << TLBMinRTT << "-" << TLBBetterPathRTT
                        << "-" << TLBPoss << "-" << TLBS << "-" << TLBT1 << "-"
                        << TLBProbingInterval << "-" << TLBSmooth << "-" << TLBRerouting << "-"
                        << quantifyRTTBase << "-";
    }
    ...
    {
      flowMonitorFilename << "clove-" << cloveRunMode << "-" << cloveFlowletTimeout << "-"
                          << cloveHalfRTT << "-" << cloveDisToUncongestedPath << "-";
      linkMonitorFilename << "clove-" << cloveRunMode << "-" << cloveFlowletTimeout << "-"
                          << cloveHalfRTT << "-" << cloveDisToUncongestedPath << "-";
    }
  else if (runMode == DRILL)
    {
  ...
  if (resequenceBuffer)
    {
      flowMonitorFilename << "rb-" << resequenceInOrderSize << "-" << resequenceInOrderTimer << "-"
                          << resequenceOutOrderTimer;
      linkMonitorFilename << "rb-" << resequenceInOrderSize << "-" << resequenceInOrderTimer << "-"
                          << resequenceOutOrderTimer;
      rbTraceFilename << "rb-";
    }
