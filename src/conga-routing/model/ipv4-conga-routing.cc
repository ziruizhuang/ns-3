...
Ipv4CongaRouting::Ipv4CongaRouting ()
    : // Parameters
      ...
      m_flowletTimeout (MicroSeconds (
          50)), // The default value of flowlet timeout is small for experimental purpose
      m_ecmpMode (false),
      // Variables
      m_feedbackIndex (0),
      m_dreEvent (),
...
{
  std::map<uint32_t, std::map<uint32_t, std::pair<Time, uint32_t> > >::iterator itr = m_congaToLeafTable.find (leafId);
...
{
  NS_LOG_LOGIC (this << " Add Conga routing entry: " << network << "/" << networkMask
                     << " would go through port: " << port);
...
bool
Ipv4CongaRouting::RouteInput (Ptr<const Packet> p, const Ipv4Header &header,
      ...
      if (!found)
        {
          ...
          std::map<uint32_t, std::map<uint32_t, FeedbackInfo> >::iterator fbItr = m_congaFromLeafTable.find (destLeafId);
          ...
          if (fbItr != m_congaFromLeafTable.end ())
            {
              ...
              if (!(innerFbItr->second).change) // prefer the changed ones
                {
                  for (unsigned loopIndex = 0; loopIndex < (fbItr->second).size ();
                       loopIndex++) // prevent infinite looping
                    {
                      if (++innerFbItr == (fbItr->second).end ())
                        {
                          innerFbItr = (fbItr->second).begin (); // start from the beginning
                        }
                      if ((innerFbItr->second).change)
                        {
                          break;
                        }
                    }
          ...
          struct Flowlet *flowlet = nullptr;
          ...
          if (flowletItr != m_flowletTable.end ())
            {
              ...
              if (flowlet != nullptr && // Impossible in normal cases
                  now - flowlet->activeTime <= m_flowletTimeout)
                {
                  ...
                  NS_LOG_LOGIC (this << " Sending Conga on leaf switch (flowlet hit): " << m_leafId
          ...
          std::map<uint32_t, std::map<uint32_t, std::pair<Time, uint32_t> > >::iterator congaToLeafItr = m_congaToLeafTable.find (destLeafId);
          ...
          for (; routeEntryItr != routeEntries.end (); ++routeEntryItr)
            {
              ...
              if (localCongestionItr != m_XMap.end ())
                {
                  localCongestion =
                      Ipv4CongaRouting::QuantizingX (port, localCongestionItr->second);
                }

              std::map<uint32_t, std::pair<Time, uint32_t> >::iterator remoteCongestionItr = (congaToLeafItr->second).find (port);
          ...
          if (flowlet != nullptr && std::find (portCandidates.begin (), portCandidates.end (),
                                               flowlet->port) != portCandidates.end ())
            {
        ...
        {
          NS_LOG_LOGIC (this << " Receiving Conga - LbTag: " << ipv4CongaTag.GetLbTag () << ", CE: "
                             << ipv4CongaTag.GetCe () << ", FbLbTag: " << ipv4CongaTag.GetFbLbTag ()
          ...
          if (ipv4CongaTag.GetFbLbTag () != LOOPBACK_PORT)
            {
              std::map<uint32_t, std::map<uint32_t, std::pair<Time, uint32_t> > >::iterator toLeafItr = m_congaToLeafTable.find (sourceLeafId);
...
void
Ipv4CongaRouting::PrintRoutingTable (Ptr<OutputStreamWrapper> stream, Time::Unit unit) const
{
}
...
{
  bool moveToIdleStatus = true;
  std::map<uint32_t, std::map<uint32_t, std::pair<Time, uint32_t> > >::iterator itr = m_congaToLeafTable.begin ();
  for (; itr != m_congaToLeafTable.end (); ++itr)
    {
      std::map<uint32_t, std::pair<Time, uint32_t> >::iterator innerItr = (itr->second).begin ();
  ...
  // OLD: for (std::map<uint32_t, std::map<uint32_t, FeedbackInfo> >::iterator itr2 = m_congaFromLeafTable.begin (); itr2 != m_congaFromLeafTable.end (); ++itr2)
  // OLD:     for (std::map<uint32_t, FeedbackInfo>::iterator innerItr2 = (itr2->second).begin (); innerItr2 != (itr2->second).end (); ++innerItr2)
  // OLD: (itr2->second).erase (innerItr2);
  // OLD: m_congaFromLeafTable.erase (itr2);
  // Fixed: by Zirui: release of container element while iteration caused iterator overrun.
  // TODO: Try const iterator
  for (auto itr2 = m_congaFromLeafTable.begin (); itr2 != m_congaFromLeafTable.end ();)
    {
      for (auto innerItr2 = (itr2->second).begin (); innerItr2 != (itr2->second).end ();)
        {
      ...
      if ((itr2->second).empty ())
        {
          itr2 = m_congaFromLeafTable.erase (itr2);
        }
      else
        {
          ++itr2;
