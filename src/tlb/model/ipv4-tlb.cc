...
Ipv4TLB::JudgePath (uint32_t destTor, uint32_t pathId)
{
    ...
    // TODO: ZZR, hand convert ns3::Time to double. Need to figure out the unit, line 1319, 1320, 1339, minRtt, m_minRtt, highRtt
    if ((m_rttAlpha * pathInfo.minRtt.GetDouble() + m_ecnBeta * consideECN * static_cast<double>(pathInfo.ecnSize) / pathInfo.size
                < m_rttAlpha * m_minRtt.GetDouble() + m_ecnBeta * consideECN * m_ecnPortionLow)
            && (pathInfo.isRetransmission) == false
            /*&& (pathInfo.isHighRetransmission) == false*/
            && (pathInfo.isTimeout) == false
            /*&& (pathInfo.isVeryTimeout) == false*/
    ...
    if ((m_rttAlpha * pathInfo.minRtt.GetDouble() + m_ecnBeta * consideECN * static_cast<double>(pathInfo.ecnSize) / pathInfo.size
                >= m_rttAlpha * m_highRtt.GetDouble() + m_ecnBeta * consideECN * m_ecnPortionHigh)
